...
public class MemoDao {
	...
	public List<MemoDto> getList(){
		...
		try {
			...
			//반복문 돌면서 select 된 할일  읽어오기
			while(rs.next()) {
				//할일을 list 에 담아 보세요.
				int num=rs.getInt("num");
				String content=rs.getString("content");
				String date=rs.getString("regdate");
				//MemoDto 객체를 생성해서 회원 한명의 정보를 담는다.
	...
	//할일을 DB 에 저장하는 메소드 (작업의 성공여부가 boolean 으로 리턴된다)
	public boolean insert(MemoDto dto) {
		...
		try {
			...
			// sql 문을 수행하고 변화된 row 의 갯수를 리턴받는다. (1)
			flag=pstmt.executeUpdate();
			System.out.println("메모를 추가 했습니다.");
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
			try {
